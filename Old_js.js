...
function tick() {
    ...
    for (let i = 0; i < mem.length; i++) {
        ...
        for (let k = 0; k < companions.length; k++) {
            ...
            if (i != j && d(mem[j], mem[i]) < InteractionDistance) {

                if (!sameIsland(i, j) && d(mem[j], mem[i]) < minDistance && mem[j][4] == mem[i][4] && (i != mem[j][2] || i != mem[j][3])) {

                    mem[mem[i][2]][3] = mem[j][3]
                    mem[mem[i][3]][2] = mem[i][2]
                    mem[mem[j][2]][3] = mem[i][3]
                    mem[mem[j][3]][2] = mem[j][2]

                    console.log("i: " + i, mem[i], "prev(i): " + mem[i][2], prev(mem[i]), "next(i): " + mem[i][3], next(mem[i]))

                    console.log("j: " + j, mem[j], "prev(j): " + mem[j][2], prev(mem[j]), "next(j): " + mem[j][3], next(mem[j]))



                    islands.push(mem[j][3])
                    islands.push(mem[i][3])
    ...
    for (let i = 0; i < islands.length; i++) {
        ...
        if(!remove){
            for (let j = i+1; j < islands.length; j++) {
                const element = islands[j];
                if (islands[j] == islands[i] || sameIsland(islands[i] , islands[j])){
                    remove = true;
                    j = islands.length
                }
            }
        ...
        if (!remove) {
            ...
            do {
                ...
                if (false && mem[current[3]] === undefined) {
                    console.log(current);
            ...
            } while (!terminate && current[3] != start)
            //current = mem[start]
            //ctx.lineTo(current[0][0], current[0][1]);
